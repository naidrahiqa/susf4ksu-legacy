...
static void print_help(void) {
	...
	log("         |--> The only difference to add_sus_path is that the added sus_path via this cli\n");
	log("         |--> will be flagged as SUS_PATH again for the app process when it is being spawned\n");
	log("         |--> by zygote and marked umounted\n");
	log("\n");
	log("        hide_sus_mnts_for_non_su_procs <0|1>\n");
	log("         |--> 0: Do NOT hide sus mounts for non-su processes (default)\n");
	log("         |--> 1: hide all sus mounts for non-su processes\n");
	log("\n");
	log("        enable_avc_log_spoofing <0|1>\n");
	log("         |--> 0: disable spoofing the sus tcontext 'su' shown in avc log\n");
	log("         |--> 1: enable spoofing the sus tcontext 'su' with 'u:r:priv_app:s0:c512,c768'\n");
	log("\n");
	log("        add_sus_map </path/to/actual/library>\n");
	log("         |--> added real file path which gets mmapped will be hidden from\n");
	log("         |--> /proc/self/[maps|smaps|smaps_rollup|map_files|mem|pagemap]\n");
	log("\n");
...
int main(int argc, char *argv[]) {
		...
		if (!strcmp(argv[2], "version")) {
			...
			if (!error) {
				...
				if (enabled_features & (1 << 13)) {
					str_len = strlen("CONFIG_KSU_SUSFS_SUS_MAP\n");
					strncpy(ptr_buf, "CONFIG_KSU_SUSFS_SUS_MAP\n", str_len);
					ptr_buf += str_len;
				}
	...
	// hide_sus_mnts_for_non_su_procs
		...
		info.err = -1;
		prctl(KERNEL_SU_OPTION, CMD_SUSFS_HIDE_SUS_MNTS_FOR_NON_SU_PROCS, &info, NULL, &error);
		if (info.err == -1)
			log("[-] CMD: '0x%x', SUSFS operation not supported, please enable it in kernel\n", CMD_SUSFS_HIDE_SUS_MNTS_FOR_NON_SU_PROCS);
		return info.err == -1 ? -1 : info.err;
	// enable_avc_log_spoofing
		...
		info.enabled = atoi(argv[2]);
		info.err = -1;
		prctl(KERNEL_SU_OPTION, CMD_SUSFS_ENABLE_AVC_LOG_SPOOFING, &info, NULL, &error);
		if (info.err == -1)
