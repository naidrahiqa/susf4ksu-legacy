...
**Universal** — tidak terikat device tertentu. Contoh consumer: Pollux Kernel (Redmi 12).  
...
Jalankan `bash pollux/apply.sh <kernel_dir>` — ini melakukan semua langkah di bawah.  
...
4. **Fixup scripts** (urut):
   ...
   python3 pollux/fix_mtk_includes.py <kernel_dir>        # Clang compat MTK headers
   ```
...
- **`.rej` files** = gagal patch. Cari dengan `pollux/verify.sh`.
...
| `pollux/verify.sh` | Verification script — jalankan setelah apply |
| `pollux/apply.sh` | Auto-apply script (bash) |

## Auto-apply script (`pollux/apply.sh`)

Mendeteksi apakah SUSFS sudah diapply (cek `fs/susfs.c`), apply main patch dengan `--fuzz=5`, fallback ke `wiggle`, lalu apply semua `patches/*.patch`, dan semua fixup scripts. **Idempotent** — skip jika `fs/susfs.c` sudah ada.
